...
ADMIN_TOKEN=$(curl -s -X POST "$API_URL/api/auth/login" \
    ...
    -d "{\"email\":\"${CARFAX_ADMIN_EMAIL:?CARFAX_ADMIN_EMAIL not set}\",\"password\":\"${CARFAX_ADMIN_PASSWORD:?CARFAX_ADMIN_PASSWORD not set}\"}" | \
    python3 -c "import sys,json; print(json.load(sys.stdin).get('access_token',''))")

TENANT_TOKEN=$(curl -s -X POST "$API_URL/api/auth/login" \
    -H "Content-Type: application/json" \
    -d "{\"email\":\"${CARFAX_TENANT_A_EMAIL:?CARFAX_TENANT_A_EMAIL not set}\",\"password\":\"${CARFAX_TENANT_A_PASSWORD:?CARFAX_TENANT_A_PASSWORD not set}\"}" | \
    python3 -c "import sys,json; print(json.load(sys.stdin).get('access_token',''))")
