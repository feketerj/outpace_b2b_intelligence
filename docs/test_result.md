...
agent_communication:
  ...
  - agent: "main"
    message: "🔧 P0 FIX COMPLETED: Made Sync Now deterministic. CHANGES: 1) Modified /api/admin/sync/{tenant_id} in admin.py to be synchronous - now awaits actual sync and returns {status, tenant_id, tenant_name, opportunities_synced, intelligence_synced, errors}. 2) Added sync_type query param (all/opportunities/intelligence). 3) Updated sync_scheduler.py to return counts. PROOF: curl test took 42s (synchronous), returned intelligence_synced:1. Frontend IntelligenceFeed.js already parses result.intelligence_synced. Also fixed Sync Now button visibility - now only shows for super_admin. carfax.sh 26/26 tests pass. Need testing agent to verify full flow."