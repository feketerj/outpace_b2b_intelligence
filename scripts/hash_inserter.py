...
def load_manifest(manifest_path: Path) -> tuple[dict, str]:
    ...
    Returns: (manifest_data, mode) where mode is "schema" or "legacy".
    """
    ...
    if isinstance(manifest, dict) and isinstance(manifest.get('artifacts'), list):
        ...
        manifest.setdefault('created', utc_timestamp())
        manifest.setdefault('updated', utc_timestamp())
...
def main():
    ...
    manifest, manifest_mode = load_manifest(manifest_path)
    ...
    if manifest_mode == 'schema':
        ...
        for filepath in sorted(manifest_files):
            ...
            existing = None
            if filepath.name in manifest_index:
                existing = artifacts[manifest_index[filepath.name]].get('sha256')

            if existing != computed:
                entry = {'path': filepath.name, 'sha256': computed}
                if filepath.name in manifest_index:
                    artifacts[manifest_index[filepath.name]] = entry
                else:
    ...
    if manifest_updated > 0:
        if manifest_mode == 'schema':
            manifest['updated'] = utc_timestamp()
