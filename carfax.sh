...
TENANT_A_ADMIN_PASSWORD="REDACTED_TEST_PASSWORD"
...
test_S0_auth_happy_expansion() {
    ...
    local admin_password="REDACTED_TEST_PASSWORD"
    ...
    resp=$(curl -s -w "\n%{http_code}" -X POST "$API_URL/api/auth/register" \
        -H "Content-Type: application/json" \
        -d "{\"email\":\"$unique_email\",\"password\":\"REDACTED_TEST_PASSWORD\",\"full_name\":\"CARFAX Test User\",\"role\":\"tenant_user\",\"tenant_id\":\"$TENANT_A_ID\"}")
...
test_S0_auth_invalid_expansion() {
    ...
    resp=$(curl -s -w "\n%{http_code}" -X POST "$API_URL/api/auth/login" \
        -H "Content-Type: application/json" \
        -d '{"email":"nonexistent-user-12345@example.com","password":"REDACTED_TEST_PASSWORD"}')
    status=$(echo "$resp" | tail -n1)
    ...
    resp=$(curl -s -w "\n%{http_code}" -X POST "$API_URL/api/auth/login" \
        -H "Content-Type: application/json" \
        -d '{"email":"not-an-email","password":"REDACTED_TEST_PASSWORD"}')
    status=$(echo "$resp" | tail -n1)
    ...
    resp=$(curl -s -w "\n%{http_code}" -X POST "$API_URL/api/auth/register" \
        -H "Content-Type: application/json" \
        -d "{\"email\":\"$ADMIN_EMAIL\",\"password\":\"REDACTED_TEST_PASSWORD\",\"full_name\":\"Duplicate User\",\"role\":\"tenant_user\",\"tenant_id\":\"$TENANT_A_ID\"}")
    status=$(echo "$resp" | tail -n1)
    local body=$(echo "$resp" | sed '$d')
...
test_S0_auth_boundary_expansion() {
    ...
    local min_password="REDACTED_TEST_PASSWORD" # Exactly 8 characters
    resp=$(curl -s -w "\n%{http_code}" -X POST "$API_URL/api/auth/register" \
        -H "Content-Type: application/json" \
    ...
    resp=$(curl -s -w "\n%{http_code}" -X POST "$API_URL/api/auth/register" \
        -H "Content-Type: application/json" \
        -d "{\"email\":\"$max_email\",\"password\":\"REDACTED_TEST_PASSWORD\",\"full_name\":\"Max Email User\",\"role\":\"tenant_user\",\"tenant_id\":\"$TENANT_A_ID\"}")
    status=$(echo "$resp" | tail -n1)
