...
# Retry-enabled HTTP client for Perplexity
_http_client = RetryableClient(timeout=60.0, max_retries=3)


@circuit_protected(perplexity_circuit)
async def sync_perplexity_intelligence(db, tenant: dict) -> int:
    ...
    Protected by circuit breaker - fails fast if Perplexity is down.
    """
    tenant_id = tenant["id"]
